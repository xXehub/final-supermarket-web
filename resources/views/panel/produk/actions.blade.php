...
<form id="delete-form-{{ $produk->id }}" action="{{ route('produk.destroy', ['produk' => $produk->id]) }}"
    ...
    <div class="btn-toolbar mb-3" role="toolbar" aria-label="Toolbar with button groups">
        <div class="btn-group me-2" role="group" aria-label="First group">
            <a type="button" href="{{ route('produk.show', ['produk' => $produk->id]) }}" class="btn btn-secondary"><i
                    class="ti-search"></i></a>
            <a type="button" href="{{ route('produk.edit', ['produk' => $produk->id]) }}" class="btn btn-secondary"><i
                    class="ti-pencil-alt"></i></a>
            @csrf
            @method('delete')
            <button type="button" class="btn btn-secondary" onclick="confirmDelete({{ $produk->id }})"><i
